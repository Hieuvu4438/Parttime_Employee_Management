...
This section transforms the entity class diagram from the Analysis Phase into the entity class diagram of the Design Phase. In the Analysis Phase, the entity classes were identified mainly by their business meaning and relationships. In the Design Phase, these classes must be refined so that they are ready for database design and detailed class design. Therefore, the design focuses on adding identifiers, specifying data types, converting relationships into appropriate object relationships, and adding object attributes that represent those relationships.
...
After adding identifiers, the next design activity is to specify the type of every attribute in each entity class. This makes the entity classes more concrete and prepares them for implementation and database mapping.
...
The Analysis Phase identifies the relationships among entity classes. In the Design Phase, these relationships are converted into clearer aggregation or composition relationships so that the object structure can be represented directly in the class diagram.
...
This association is converted into two composition relationships. First, Employee becomes a component of RegistrationShift, because each registration record must contain the selected employee. Second, Shift becomes a component of RegistrationShift, because each registration record must also contain the selected shift. In the design entity class diagram, both filled diamonds are placed at RegistrationShift, showing that RegistrationShift is composed from the selected Employee and the selected Shift.


\subsection{Add Object Attributes Based on Aggregation and \\ Composition Relationships}

After converting the relationships, object attributes are added to represent those relationships inside the corresponding classes.
...
Employee is a component of RegistrationShift, with a 1--n relationship from Employee to RegistrationShift. Therefore, each RegistrationShift has one employee, represented by the object attribute employee: Employee.

Shift is a component of RegistrationShift, with a 1--n relationship from Shift to RegistrationShift. Therefore, each RegistrationShift has one shift, represented by the object attribute shift: Shift.

User is aggregated by RegistrationShift, with a 1--n relationship from User to RegistrationShift. Therefore, each RegistrationShift has one user, represented by the object attribute user: User. This attribute records which registration staff created the shift registration.
...
This section designs the database diagram from the entity class diagram in the Design Phase. The purpose of this part is to transform persistent entity classes into relational tables, remove object attributes that cannot be stored directly as table columns, and configure primary keys and foreign keys so that the relationships among tables are represented correctly in the database.
...
\subsection{Represent Relationships Between Tables}

The database diagram keeps the main relationships from the design entity class diagram and represents them through table relationships. The relationship between tblRestaurant and tblEmployee is a one-to-many relationship because one restaurant can manage many employees, while each employee belongs to one restaurant.

The table tblRegistrationShift is the central table for the registration function. It connects tblEmployee and tblShift, because each registration record is created for one selected employee and one selected shift. It also connects to tblUser, because each registration record must store the staff account that performs the registration operation.
...
The DAO classes form the control and data access part of the design. They separate database operations from the user interfaces, so the form classes do not directly execute database queries. The general class DAO contains the common database connection attribute con: Connection and the constructor DAO(). The specific DAO classes inherit from DAO and implement operations related to their corresponding entity classes.

For each data access operation required by the View classes, the corresponding DAO method is designed by analyzing the operation description, input parameter candidates, output parameter candidates, and DAO class assignment. The assignment rule is to prioritize the class related to the output type first; if the output is primitive or not related to an entity class, the input type is used to decide the DAO class.
...
The output needs to convey whether login is correct and also allow the system to know the staff role after login. There are three output candidates: boolean, String, and User. Returning String only provides the position or role and does not clearly express success or failure. Returning User can contain full information, but the instructor's sample diagrams commonly use boolean for login checking. Therefore, the selected output is boolean. In Java, objects are passed by reference, so the DAO method can populate the User object's fields when a matching account is found, then return true or false as the login result.

The output is boolean, which is a primitive type and has no corresponding DAO class. Therefore, the assignment is based on the input object. Since the input object is User, this operation is assigned to UserDAO. The final signature is UserDAO.checkLogin(User u): boolean.
...
The output is ArrayList<Employee>, which is related to Employee. Therefore, this operation is assigned to EmployeeDAO. The final signature is EmployeeDAO.searchEmployee(String key): ArrayList<Employee>.
...
The output is ArrayList<Shift>, which is related to Shift. Therefore, this operation is assigned to ShiftDAO. The final signature is ShiftDAO.getShiftNextWeek(): ArrayList<Shift>.
...
The output is ArrayList<RegistrationShift>, which is related to RegistrationShift. Therefore, this operation is assigned to RegistrationShiftDAO. The final signature is RegistrationShiftDAO.getRegistrationByEmployee(Employee e): ArrayList<RegistrationShift>.
...
In the Entity layer, Restaurant has a composition relationship with Employee, because a restaurant manages many employees. RegistrationShift has composition relationships with Employee and Shift, because each registration record is composed from the selected employee and the selected shift. RegistrationShift also has an aggregation relationship with User, because the user account that creates the registration can exist independently from the registration record.
...
This section presents Scenario v.3 for the standard scenario of Module 1, \textit{Register for next week}. In the Design Phase, the scenario is written at a more detailed level than the Analysis Phase. It shows not only the interaction between the actors and the interfaces, but also the internal calls among boundary classes, entity classes, and DAO classes. Therefore, this scenario is suitable for building the sequence diagram of the module.

The main external actors in this scenario are the part-time employee and the registration staff. The part-time employee provides the information about the desired shifts, while the registration staff operates the system through the design interface classes. The main design classes participating in the scenario are LoginFrm, HomeFrm, SearchEmployeeFrm, RegisterShiftFrm, User, Employee, Shift, RegistrationShift, UserDAO, EmployeeDAO, ShiftDAO, and RegistrationShiftDAO.
...
\begin{enumerate}
    ...
    \item The method checkLogin() calls the class User to set more user attributes, such as fullName and position.
    \item The class User calls its setter methods setFullName() and setPosition().
    ...
    \item The method actionPerformed() of SearchEmployeeFrm is called.
    \item The method actionPerformed() calls the method getShiftNextWeek() of the class ShiftDAO.
    \item The method getShiftNextWeek() executes the operation for retrieving next week's shifts.
    \item The method getShiftNextWeek() calls the class Shift to pack the results.
    \item The class Shift packs each result into a Shift object.
    \item The class Shift returns the Shift objects to the method getShiftNextWeek().
    \item The method getShiftNextWeek() returns the shift results to the method actionPerformed().
    ...
    \item The class RegistrationShift packs each ticked shift into a RegistrationShift object with the selected employee, selected shift, logged-in user, registered time, and status.
    ...
    \item The method saveRegistration() executes the database save operation.
    \item The method saveRegistration() returns the save result as a boolean value to the method actionPerformed().
