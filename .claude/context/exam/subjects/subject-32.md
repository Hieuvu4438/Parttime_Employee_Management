...
Module "View the racers' standings ": A staff selects the statistics function → Select to view the  
current racer rankings → The staff selects a stage from the dropdown list -> The system displays a  
list of racers in the form of a table, each line contains: Racer's name, nationality, team name, total  
score after the selected stages, total time after stages (sorted in descending order of total score, then  
in ascending order of total time). The staff clicks on a line of a racer → the system displays the  
detailed results of each race stage given by that racer, each stage on one line: stage name, finish  
rank, score, time to finish.
