...
interface to choose the periode of statistics (start - end date) appears → After selecting the periode,  
the staff click statistics → the results appear including a list of detailed dishes: code, type, name,  
total number of sales, total revenue (Sorted by total revenue, from high to low). The staff clicks on a  
line of a dish, the system displays a detailed list of times the dish was ordered: id, guest name, date  
and time, quantity, amount.
