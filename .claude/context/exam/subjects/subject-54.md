...
year) → the system displays a box to select statistics by month, quarter, or year →  clicka by month  
→ the system displays revenue statistics for the last 12 months in the form of a table, each line  
corresponds to 1 month (corresponding to quarter, year): month name, total revenue, sorted by time.  
64

The staff clicks on 1 line of the results → the system details the customer's invoices during the click  
line, each invoice on 1 line: id, customer name, court name, date and time, total payment.
