...
| Bước | Thao tác | Mô tả |
|------|----------|-------|
| 1 | Mở Visual Paradigm → New → Class Diagram | Tạo diagram mới, đặt tên "Library_StatReaders" |
| 2 | Tạo entity class boxes | Kéo "Class" từ toolbar vào canvas, tạo 5 class: Book, Reader, Loan, LoanDetail, User |
| 3 | Tạo view class boxes | Kéo "Boundary" vào canvas, tạo: LoginFrm, HomeFrm, StatReaderFrm |
...
| Ngăn | Nội dung | Ví dụ (class Reader) |
|------|----------|----------------------|
| Ngăn 1 — Tên class | Stereotype + tên class | `<<entity>> Reader` |
| Ngăn 2 — Thuộc tính | `-attributeName: Type` | `-readerId: int`, `-code: String`, `-name: String`, `-dob: Date`, `-address: String`, `-barcode: String` |
| Ngăn 3 — Phương thức | `+methodName(params): ReturnType` | `+countBooksByReader(start: Date, end: Date): List<Map>` |
...
| StatReaderFrm | `<<boundary>>` | `-inStartDate: JTextField`, `-inEndDate: JTextField`, `-subSearch: JButton`, `-outReaderTable: JTable`, `-outDetailLabel: JLabel`, `-outDetailTable: JTable`, `-subExport: JButton`, `-subBack: JButton` |
...
| **Dependency** | Đường dashed, mũi tên tam giác rỗng (▷) | "Sử dụng" tạm thời (StatReaderFrm → ReaderDAO) |
...
| 0..1 | Ghi "0..1" | (hiếm dùng trong module này) |
...
*Ví dụ 1: Vẽ quan hệ Reader → Loan (1-n, Association)*
...
*Ví dụ 2: Vẽ quan hệ Loan → LoanDetail (1-n, Composition)*
