...
2. Tạo entity class boxes (hình chữ nhật 3 ngăn) cho từng entity: FoodItem, FoodInvoice, FoodInvoiceDetail, User.
3. Tạo view class boxes từ các interface: HomeFrm, ServiceStatFrm.
...
- **Ngăn 2 (thuộc tính):** Mỗi thuộc tính ghi dạng `-attributeName: Type`. Ví dụ: `-code: String`, `-price: double`.
- **Ngăn 3 (phương thức):** Mỗi phương thức ghi dạng `+methodName(params): ReturnType`. Ví dụ: `+getServiceStat(startDate: Date, endDate: Date): List<ServiceStat>`.

**Bảng chi tiết từng entity class:**

| Class | Stereotype | Attributes | Methods |
|-------|-----------|------------|---------|
| FoodItem | `<<entity>>` | -id: int, -code: String, -name: String, -price: double, -size: String, -description: String | +getServiceStat(startDate: Date, endDate: Date): List<ServiceStat> |
| FoodInvoice | `<<entity>>` | -id: int, -invoiceDate: Date, -totalAmount: double | getter/setter |
| FoodInvoiceDetail | `<<entity>>` | -id: int, -foodInvoiceId: int, -foodItemId: int, -quantity: int, -unitPrice: double, -amount: double | +getServiceDetail(foodItemId: int, startDate: Date, endDate: Date): List<ServiceDetail> |
...
|------------|-----------|-------------|
| HomeFrm | `<<boundary>>` | subStatistics: JButton |
| ServiceStatFrm | `<<boundary>>` | inStartDate: JTextField, inEndDate: JTextField, subView: JButton, outServiceTable: JTable (clickable), outDetailTable: JTable |

**Cách vẽ quan hệ:**

- **Association** (đường liền nét, mũi tên tam giác rỗng ▷): dùng cho quan hệ tham chiếu thông thường. Ví dụ: FoodItem → FoodInvoiceDetail.
- **Composition** (đường liền nét, đầu kim cương filled ◆): dùng cho "contain" nhưng child KHÔNG tồn tại nếu không có parent. Ví dụ: FoodInvoice ◆→ FoodInvoiceDetail.
- **Dependency** (đường dashed, mũi tên tam giác rỗng ▷): dùng cho "sử dụng" tạm thời. Ví dụ: ServiceStatFrm → FoodItemDAO.

**Cách ghi multiplicity:**

- 1..1 → ghi "1" ở một đầu.
- 0..* hoặc 1..* → ghi "n" hoặc "*" ở đầu kia.
- Ghi ở cả 2 đầu của đường kết nối. Ví dụ: FoodInvoice "1" --- "n" FoodInvoiceDetail.

**Bảng quan hệ chi tiết:**

| Từ | Đến | Kiểu quan hệ | Multiplicity | Giải thích |
|----|-----|--------------|--------------|------------|
| FoodInvoice | FoodInvoiceDetail | Composition | 1 — n | Một hóa đơn có nhiều chi tiết, chi tiết không tồn tại nếu không có hóa đơn |
| FoodItem | FoodInvoiceDetail | Association | 1 — n | Một mặt hàng xuất hiện trong nhiều chi tiết hóa đơn |
| User | FoodInvoice | Association | 1 — n | Một nhân viên lập nhiều hóa đơn |
| ServiceStatFrm | FoodItemDAO | Dependency | — | Frm sử dụng FoodItemDAO để thống kê dịch vụ |
| ServiceStatFrm | FoodInvoiceDetailDAO | Dependency | — | Frm sử dụng FoodInvoiceDetailDAO để lấy chi tiết bán hàng |

**Ví dụ cụ thể trên Visual Paradigm:**

1. **Vẽ quan hệ FoodInvoice → FoodInvoiceDetail (Composition 1-n):**
   - Kéo class FoodInvoice lên canvas, kéo class FoodInvoiceDetail bên dưới.
   - Chọn tool "Association" → click vào FoodInvoice, kéo đến FoodInvoiceDetail.
   - Đặt multiplicity "1" phía FoodInvoice, "n" phía FoodInvoiceDetail.
   - Click chuột phải → "Association End" → phía FoodInvoice đặt "filled diamond" (◆).

2. **Vẽ dependency ServiceStatFrm → FoodItemDAO:**
   - Chọn tool "Dependency" (đường dashed) → click vào ServiceStatFrm, kéo đến FoodItemDAO.
...
Chọn Statistics -> Service statistics -> Giao diện thống kê dịch vụ xuất hiện:
- ô nhập ngày bắt đầu -> inStartDate
- ô nhập ngày kết thúc -> inEndDate
- nút View -> subView
- bảng danh sách dịch vụ (click được) -> outServiceTable
- bảng chi tiết bán hàng -> outDetailTable

Nhập ngày và nhấn View -> hệ thống thống kê dịch vụ -> cần phương thức:
- tên: getServiceStat()
- đầu vào: startDate, endDate
- đầu ra: danh sách ServiceStat
- gán cho entity class: FoodItem.

Nhấn vào dòng dịch vụ -> hệ thống lấy chi tiết bán hàng -> cần phương thức:
- tên: getServiceDetail()
- đầu vào: foodItemId, startDate, endDate
- đầu ra: danh sách ServiceDetail
- gán cho entity class: FoodInvoiceDetail.

#### Tóm tắt
View classes: HomeFrm, ServiceStatFrm
Methods: getServiceStat(), getServiceDetail()
...
|--------|------|------------|
| FoodItem | Model | id: int (PK), code: String, name: String, price: float, size: String, description: String |
| FoodInvoice | Model | id: int (PK), invoiceDate: DateTime, totalAmount: float, user: User |
| FoodInvoiceDetail | Model | id: int (PK), quantity: int, unitPrice: float, amount: float, foodInvoice: FoodInvoice, foodItem: FoodItem |
