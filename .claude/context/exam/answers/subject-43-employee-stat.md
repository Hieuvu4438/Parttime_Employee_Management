...
| 3 | Tạo các view class box từ giao diện: HomeFrm, EmployeeStatFrm |
...
- **Ngăn 1 (tên class):** Ghi stereotype `<<entity>>` hoặc `<<boundary>>` rồi đến tên class. Ví dụ: `<<entity>> Wage`, `<<boundary>> EmployeeStatFrm`
- **Ngăn 2 (thuộc tính):** Ghi từng thuộc tính theo format `-tenThuocTinh: KieuDuLieu`. Dùng `-` cho private. Ví dụ: `-totalShiftHours: float`, `-totalReceived: float`
- **Ngăn 3 (phương thức):** Ghi từng phương thức theo format `+tenPhuongThuc(thamSo): KieuTraVe`. Dùng `+` cho public. Ví dụ: `+getEmployeeStat(startDate: Date, endDate: Date): List<EmployeeStat>`
...
| Employee | `<<entity>>` | `-id: int`, `-code: String`, `-fullName: String`, `-phoneNumber: String`, `-email: String`, `-dob: Date`, `-contractDate: Date`, `-restaurantID: int` | `+searchEmployee(keyword: String): List<Employee>`, `+getEmployeeStat(startDate: Date, endDate: Date): List<EmployeeStat>` |
...
| Attendance | `<<entity>>` | `-id: int`, `-employeeID: int`, `-shiftID: int`, `-checkinTime: DateTime`, `-checkoutTime: DateTime` | `+getShiftHours(): float`, `+getOvertime(): float`, `+getLateHours(): float`, `+getAttendanceDetail(employeeId: int, startDate: Date, endDate: Date): List<AttendanceDetail>` |
| Wage | `<<entity>>` | `-id: int`, `-employeeID: int`, `-weekStart: Date`, `-weekEnd: Date`, `-totalShiftHours: float`, `-totalOvertime: float`, `-totalLateHours: float`, `-totalReceived: float` | — |
...
| `outsubListEmployee` | outsub | JTable | Bảng thống kê NV (click được) |
...
1. Chọn công cụ **Aggregation** (đường liền nét đầu kim cương rong ◇) từ toolbox.
...
**Ví dụ 2: Vẽ quan hệ Shift → Attendance (Association 1-n)**

1. Chọn công cụ **Association** (đường liền nét mũi tên rỗng) từ toolbox.
2. Click vào `Shift`, kéo sang `Attendance`.
3. Click chuột phải → **Open Specification**.
4. Tại mục **From** (Shift): multiplicity = `1`, role name = `recordedFor`.
5. Tại mục **To** (Attendance): multiplicity = `*`.
6. Nhấn OK.
...
| Wage | Model | id: int (PK), weekStart: Date, weekEnd: Date, totalShiftHours: float, totalOvertime: float, totalLateHours: float, totalReceived: float, employee: Employee |
