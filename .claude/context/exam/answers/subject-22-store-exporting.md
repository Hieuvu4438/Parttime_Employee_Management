...
| Ma hang, ten hang, mo ta, ton kho | Attribute (Item) | Thuộc tính của Item |
...
| Item | `<<entity>>` | `-itemId: String`, `-itemName: String`, `-description: String`, `-stockQuantity: int` | `+searchItemByName(name: String): List<Item>`, `+getStock(itemId: String): int`, `+updateStock(itemId: String, qty: int): boolean` |
...
| ExportInvoice | `<<entity>>` | `-invoiceId: String`, `-exportDate: Date`, `-totalAmount: float`, `-agent: SubAgent`, `-user: User` | `+insertExportInvoice(inv: ExportInvoice): String` |
| ExportInvoiceDetail | `<<entity>>` | `-detailId: String`, `-exportInvoice: ExportInvoice`, `-item: Item`, `-quantity: int`, `-unitPrice: float`, `-amount: float` | `+insertExportInvoiceDetails(details: List): boolean` |
...
|--------|------|------------|
| Item | Entity | id: int (PK), itemId: String, itemName: String, description: String, stockQuantity: int |
| SubAgent | Entity | id: int (PK), agentId: String, agentName: String, address: String, phone: String |
| ExportInvoice | Entity | id: int (PK), invoiceId: String, exportDate: Date, totalAmount: float, agent: SubAgent (FK), user: User (FK) |
| ExportInvoiceDetail | Entity | id: int (PK), exportInvoice: ExportInvoice (FK), item: Item (FK), quantity: int, unitPrice: float, amount: float |
...
|--------|--------------|------------|
| Item | Entity Class | itemId: String, itemName: String, description: String, stockQuantity: int |
| SubAgent | Entity Class | agentId: String, agentName: String, address: String, phone: String |
| ExportInvoice | Entity Class | invoiceId: String, exportDate: Date, totalAmount: float, agent: SubAgent (FK), user: User (FK) |
| ExportInvoiceDetail | Entity Class | detailId: String, exportInvoice: ExportInvoice (FK), item: Item (FK), quantity: int, unitPrice: float, amount: float |
...
|--------|----------|-------------|---------------|
| IT012 | Sua Vinamilk | Sua tuoi 1 lit | 500 |
| IT023 | Banh Oreo | Banh quy 300g | 200 |
| IT034 | Nuoc ngot Coca | Chai 1.5 lit | 300 |
...
| 2 | Staff nhap "Minh Khang" vao o tim kiem dai ly, nhan nut Tim kiem | "Minh Khang" | Bang danh sach đại lý hiển thị 1 dòng: AG005, Dai ly Minh Khang, 123 Le Loi, Q.1, TP.HCM, 0901234567 |
| 3 | Staff nhan chon dong dai ly AG005 | Chon AG005 | Thong tin dai ly hien thi: Ten = "Dai ly Minh Khang", DC = "123 Le Loi, Q.1, TP.HCM", SDT = "0901234567". O tim kiem hang hoa duoc kich hoat. |
| 4 | Staff nhap "Vinamilk" vao o tim kiem hang hoa, nhan nut Tim kiem | "Vinamilk" | Bang danh sach hàng hóa hiển thị 1 dòng: IT012, Sua Vinamilk, Sua tuoi 1 lit, Ton kho: 500 |
| 5 | Staff chon IT012, nhap SL=50, Don gia=250000, nhan nut Them vao danh sach | SL=50, Don gia=250000 | Bảng xuất có 1 dòng: IT012, Sua Vinamilk, 50, 250,000, 12,500,000. Tổng tiền = "12,500,000 VND". Các ô nhập được xóa trắng. |
| 6 | Staff tim "Oreo", chon IT023, nhap SL=30, Don gia=180000, nhan nut Them | SL=30, Don gia=180000 | Bảng xuất có 2 dòng. Dong 2: IT023, Banh Oreo, 30, 180,000, 5,400,000. Tổng tiền = "17,900,000 VND". |
| 7 | Staff tim "Coca", chon IT034, nhap SL=100, Don gia=120000, nhan nut Them | SL=100, Don gia=120000 | Bảng xuất có 3 dòng. Dong 3: IT034, Nuoc ngot Coca, 100, 120,000, 12,000,000. Tổng tiền = "29,900,000 VND". |
| 8 | Kiểm tra DB trước khi Submit | -- | tblExportInvoice vẫn trống (chưa có hóa đơn mới). tblExportInvoiceDetail vẫn trống. Tồn kho IT012=500, IT023=200, IT034=300 (chưa thay đổi). |
| 9 | Staff nhan nut Xuat hoa don | -- | Hop thoai xac nhan: "Xuat hoa don cho Dai ly Minh Khang voi tong tien 29,900,000 VND?" |
| 10 | Staff nhan "Xac nhan" | -- | Thong bao "Xuat hoa don thanh cong!". Phieu xuat hoa don duoc in ra. |
| 11 | Staff nhan OK | -- | Form reset ve trang thai ban dau. Bang danh sách xuất trống. Tổng tiền = "0 VND". |
...
Bang `tblItem` (cap nhat ton kho — chi update cot stockQuantity):
