...
2. Tạo entity class boxes (hình chữ nhật 3 ngăn) cho từng entity: Court, Customer, Booking, BookingSession, Product, SessionProduct, Supplier, ImportInvoice, ImportInvoiceDetail, Payment, User.
3. Tạo view class boxes từ các interface: HomeFrm, BookingFrm.
...
- **Ngăn 1 (tên class):** Ghi stereotype `<<entity>>`, `<<boundary>>`, hoặc `<<control>>` phía trên tên class. Ví dụ: `<<entity>> Booking`.
- **Ngăn 2 (thuộc tính):** Mỗi thuộc tính ghi dạng `-attributeName: Type`. Ví dụ: `-startDate: Date`, `-totalAmount: double`.
- **Ngăn 3 (phương thức):** Mỗi phương thức ghi dạng `+methodName(params): ReturnType`. Ví dụ: `+searchAvailableCourts(timeSlot: String, courtType: String): List<Court>`.

**Bảng chi tiết từng entity class:**

| Class | Stereotype | Attributes | Methods |
|-------|-----------|------------|---------|
| Court | `<<entity>>` | -id: int, -code: String, -name: String, -type: String, -maxGuests: int, -description: String | +searchAvailableCourts(timeSlot: String, courtType: String): List<Court> |
| Customer | `<<entity>>` | -id: int, -code: String, -fullName: String, -phoneNumber: String, -address: String | +searchCustomer(keyword: String): List<Customer> |
| Booking | `<<entity>>` | -id: int, -startDate: Date, -endDate: Date, -timeSlot: String, -dayOfWeek: String, -totalSessions: int, -totalAmount: double, -deposit: double | +createBooking(booking: Booking): int |
| BookingSession | `<<entity>>` | -id: int, -sessionDate: Date, -status: String | +addBookingSessions(bookingId: int, sessionDates: List<Date>): boolean |
| Product | `<<entity>>` | -id: int, -code: String, -name: String, -price: double | getter/setter |
| SessionProduct | `<<entity>>` | -id: int, -quantity: int, -amount: double | getter/setter |
| Supplier | `<<entity>>` | -id: int, -code: String, -name: String, -address: String, -email: String, -phone: String, -description: String | getter/setter |
| ImportInvoice | `<<entity>>` | -id: int, -importDate: Date, -totalAmount: double | getter/setter |
| ImportInvoiceDetail | `<<entity>>` | -id: int, -quantity: int, -unitPrice: double, -amount: double | getter/setter |
| Payment | `<<entity>>` | -id: int, -paymentDate: Date, -rentalAmount: double, -foodAmount: double, -totalAmount: double | getter/setter |
...
|------------|-----------|-------------|
| HomeFrm | `<<boundary>>` | subBooking: JButton |
| BookingFrm | `<<boundary>>` | inTimeSlot: JTextField, inCourtType: JComboBox, subSearch: JButton, outsubCourtTable: JTable (clickable), inCustomerName: JTextField, subSearchCustomer: JButton, outsubCustomerTable: JTable (clickable), subAddCustomer: JButton, inStartDate: JTextField, inEndDate: JTextField, inDayOfWeek: JComboBox, inTimeSlotDetail: JComboBox, outBookingSlip: JPanel, subConfirm: JButton |
...
- **Dependency** (đường dashed, mũi tên tam giác rỗng ▷): dùng cho "sử dụng" tạm thời. Ví dụ: BookingFrm → CourtDAO.
...
|----|-----|--------------|--------------|------------|
| Court | Booking | Association | 1 — n | Một sân có nhiều phiếu đặt |
| Customer | Booking | Association | 1 — n | Một khách hàng có nhiều phiếu đặt |
| Booking | BookingSession | Composition | 1 — n | Một phiếu đặt có nhiều phiên, phiên không tồn tại nếu không có phiếu đặt |
| BookingSession | SessionProduct | Composition | 1 — n | Một phiên có nhiều sản phẩm sử dụng |
| Product | SessionProduct | Association | 1 — n | Một sản phẩm được dùng trong nhiều phiên |
| Supplier | ImportInvoice | Association | 1 — n | Một nhà cung cấp có nhiều hóa đơn nhập |
| ImportInvoice | ImportInvoiceDetail | Composition | 1 — n | Hóa đơn nhập có nhiều chi tiết |
| Product | ImportInvoiceDetail | Association | 1 — n | Một sản phẩm xuất hiện trong nhiều chi tiết nhập |
| Booking | Payment | Association | 1 — n | Một phiếu đặt có thể thanh toán nhiều lần |
| User | Booking | Association | 1 — n | Một nhân viên tạo nhiều phiếu đặt |
| BookingFrm | CourtDAO | Dependency | — | Frm sử dụng CourtDAO để tìm sân trống |
| BookingFrm | CustomerDAO | Dependency | — | Frm sử dụng CustomerDAO để tìm khách hàng |
| BookingFrm | BookingDAO | Dependency | — | Frm sử dụng BookingDAO để tạo phiếu đặt |
...
2. **Vẽ dependency BookingFrm → CourtDAO:**
   - Chọn tool "Dependency" (đường dashed) → click vào BookingFrm, kéo đến CourtDAO.
   - Mũi tên tam giác rỗng (▷) tự động hiển thị phía CourtDAO.
...
Sau khi đăng nhập thành công -> HomeFrm xuất hiện:
  một lựa chọn để đặt sân -> subBooking

Chọn Booking -> BookingFrm xuất hiện:
  ô nhập khung giờ -> inTimeSlot
  combobox chọn loại sân -> inCourtType
  nút Search -> subSearch
  bảng kết quả sân trống (click được) -> outsubCourtTable

Nhập khung giờ, chọn loại sân, nhấn Search -> cần phương thức:
  tên: searchAvailableCourts()
  đầu vào: timeSlot, courtType
  đầu ra: List<Court>
  gán cho entity class: Court.

Nhấn vào sân "S1" -> hiển thị giao diện tìm khách hàng:
  ô nhập tên khách hàng -> inCustomerName
  nút Search -> subSearchCustomer
  bảng kết quả khách hàng (click được) -> outsubCustomerTable
  nút Add New Customer -> subAddCustomer
...
Nhấn vào khách hàng "Nguyen Van A" -> hiển thị form đặt sân:
  ô nhập ngày bắt đầu -> inStartDate
  ô nhập ngày kết thức -> inEndDate
  combobox chọn ngày trong tuần -> inDayOfWeek
  combobox chọn khung giờ -> inTimeSlotDetail
  phiếu đặt sân -> outBookingSlip
  nút Confirm -> subConfirm

Nhập ngày, chọn ngày trong tuần, chọn khung giờ, nhấn Confirm (lần 1) -> cần phương thức:
  tên: calculateBooking()
  đầu vào: startDate, endDate, dayOfWeek, timeSlot
  đầu ra: thông tin phiếu đặt (số tuần, tổng tiền, tiền cọc)
  gán cho: tính toán nội bộ.

Nhấn Confirm (lần 2) -> cần phương thức:
  tên: createBooking()
  đầu vào: courtId, customerId, startDate, endDate, timeSlot, dayOfWeek, totalSessions, totalAmount, deposit, userId
  đầu ra: bookingId
  gán cho entity class: Booking.

Sau khi tạo booking -> cần phương thức:
  tên: addBookingSessions()
  đầu vào: bookingId, sessionDates
  đầu ra: boolean
  gán cho entity class: BookingSession.

### Summary
View classes: HomeFrm, BookingFrm
Methods: searchAvailableCourts(), searchCustomer(), calculateBooking(), createBooking(), addBookingSessions()
...
|--------|------|------------|
| Court | Entity | id: int (PK), code: String, name: String, type: String, maxGuests: int, description: String |
| Customer | Entity | id: int (PK), code: String, fullName: String, phoneNumber: String, address: String |
| Booking | Entity | id: int (PK), startDate: Date, endDate: Date, timeSlot: String, dayOfWeek: String, totalSessions: int, totalAmount: double, deposit: double, court: Court, customer: Customer, user: User |
| BookingSession | Entity | id: int (PK), sessionDate: Date, status: String, booking: Booking |
| Product | Entity | id: int (PK), code: String, name: String, price: double |
| SessionProduct | Entity | id: int (PK), quantity: int, amount: double, bookingSession: BookingSession, product: Product |
...
| Payment | Entity | id: int (PK), paymentDate: Date, rentalAmount: double, foodAmount: double, totalAmount: double, booking: Booking |
