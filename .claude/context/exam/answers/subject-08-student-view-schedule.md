...
| Student | `<<entity>>` | `-id: int`, `-studentCode: String`, `-name: String`, `-dob: Date`, `-course: String`, `-address: String` | `+getSchedule(semester: String): List<Registration>` |
| Subject | `<<entity>>` | `-id: int`, `-code: String`, `-name: String`, `-credits: int` | `+getSubjectById(id: int): Subject` |
| Class | `<<entity>>` | `-id: int`, `-classCode: String`, `-className: String`, `-classroom: String`, `-maxStudents: int`, `-subjectID: int`, `-timeSlotID: int` | `+getClassById(id: int): Class` |
...
| ViewScheduleFrm | `<<boundary>>` | `-inViewType: JComboBox`, `-outScheduleTable: JTable`, `-outDayHeaders: JLabel[]`, `-outPeriodLabels: JLabel[]`, `-outSlotDetail: JPanel`, `-outSubjectCode: JLabel`, `-outSubjectName: JLabel`, `-outClassCode: JLabel` |
...
SV chọn View schedule → Giao diện xem thời khóa biểu xuất hiện → cần class: ViewScheduleFrm
  ...
  Hiển thị giảng viên → outTeacher
